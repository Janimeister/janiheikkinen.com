...
const NAV_ROUTES = [
  ...
  { label: 'Pet', path: '/tamagotchi', heading: 'Virtual Pet' },
] as const;
...
test.describe('Tamagotchi Page', () => {
  test.beforeEach(async ({ page }) => {
    // Clear any persisted pet so each test starts from the hatching screen.
    await page.addInitScript(() => {
      try { localStorage.removeItem('tamagotchi-pet-v1'); } catch { /* ignore */ }
    });
  ...
  test('shows mystery egg and hatch button', async ({ page }) => {
    await expect(page.locator('[data-testid="tama-hatch"]')).toBeVisible();
    await expect(page.locator('[data-testid="tama-hatch-btn"]')).toBeVisible();
  });

  test('hatch button is disabled without a name', async ({ page }) => {
    await expect(page.locator('[data-testid="tama-hatch-btn"]')).toBeDisabled();
  });

  test('hatching reveals a pet with stats and care actions', async ({ page }) => {
    await page.fill('[data-testid="tama-name-input"]', 'Fluffy');
    await page.click('[data-testid="tama-hatch-btn"]');

    // Hatch card disappears, pet name is shown
    await expect(page.locator('[data-testid="tama-hatch"]')).not.toBeVisible();
    await expect(page.locator('[data-testid="tama-pet-name"]')).toContainText('Fluffy');
    await expect(page.locator('[data-testid="tama-pet-stage"]')).toBeVisible();
    await expect(page.locator('[data-testid="tama-sprite"]')).toBeVisible();

    // Stats and care action buttons are present
    await expect(page.locator('[data-testid="tama-stats"]')).toBeVisible();
    for (const id of ['tama-feed-btn', 'tama-play-btn', 'tama-clean-btn', 'tama-sleep-btn', 'tama-heal-btn']) {
      await expect(page.locator(`[data-testid="${id}"]`)).toBeVisible();
    }
  });

  test('feeding the pet logs an event', async ({ page }) => {
    await page.fill('[data-testid="tama-name-input"]', 'Biscuit');
    await page.click('[data-testid="tama-hatch-btn"]');

    await page.click('[data-testid="tama-feed-btn"]');
    const log = page.locator('[data-testid="tama-log"]');
    await expect(log).toBeVisible();
    await expect(log).toContainText('Biscuit');
  });

  test('sleep button toggles wake label', async ({ page }) => {
    await page.fill('[data-testid="tama-name-input"]', 'Nap');
    await page.click('[data-testid="tama-hatch-btn"]');

    const sleepBtn = page.locator('[data-testid="tama-sleep-btn"]');
  ...
  test('reset returns to the egg hatching screen', async ({ page }) => {
    await page.fill('[data-testid="tama-name-input"]', 'Ephemeral');
    await page.click('[data-testid="tama-hatch-btn"]');
    await expect(page.locator('[data-testid="tama-pet-name"]')).toBeVisible();

    await page.click('[data-testid="tama-reset-btn"]');
    await expect(page.locator('[data-testid="tama-hatch"]')).toBeVisible();
  });
